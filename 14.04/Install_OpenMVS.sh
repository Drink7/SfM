...
cd SfM && mkdir -p OpenMVS && cd OpenMVS
#main_path=`pwd`
...
cmake ../ceres-solver-1.10.0 -DCMAKE_C_FLAGS=-fPIC -DCMAKE_CXX_FLAGS=-fPIC -DBUILD_EXAMPLES=OFF -DBUILD_TESTING=OFF -DMINIGLOG=ON
sudo make install
...
git clone https://github.com/glfw/glfw && mkdir -p glfw_build && cd glfw_build && cmake ../glfw && make -j4 && sudo make install && cd ..
